...
import { Architecture, computeArchitecture, stronglyConnected } from './metrics';
export type { Architecture, PastaShape } from './metrics';
...
export interface GraphData { nodes: ProjectNode[]; edges: Edge[]; cycles: number[][]; unresolved: {project: string; reference: string}[]; diagnostics: string[]; labelPrefixes: string[]; metrics: Architecture }
...
export function analyzeProjects(inputs: ProjectInput[], resolve: (project: ProjectInput, reference: string) => string, options: AnalyzerOptions = {}): GraphData {
  ...
  const data: Omit<GraphData,'metrics'> = { nodes: [], edges: [], cycles: [], unresolved: [], diagnostics: [], labelPrefixes: options.labelPrefixes ?? [] };
  ...
  const components=stronglyConnected(data.nodes.length,adjacency);
  ...
  return {...data,metrics:computeArchitecture(data)};
}
